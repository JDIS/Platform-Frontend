...
const ResultPage = React.createClass({
  ...
  renderTests() {
    return this.state.result.results.map(test => {
      ...
      let compilation = test.isCompilationError ? <span><Glyphicon glyph="trash" /> (compilation failed)</span> : '';
      return <li className={className}><Glyphicon glyph={glyph} /> {test.name} {timeout}{compilation}</li>
    });
  },
    ...
    if(!this.state.resultLoaded) {
      return (
        <div>
          ...
          <h2 className='text-center'>Tests en cours, veuillez patienter...</h2>
        </div>
      )
    }
    ...
    return (
      <div className="row">
        ...
        <Jumbotron style={{padding: '20px', marginLeft: 'auto', marginRight: '20px', width: '312px', position: 'relative'}} className='well'>
          <h3 style={{marginTop: '0px', fontWeight: 'bold', fontSize: '16px'}}>Résultat</h3>
          <Doughnut data={chartData} width="250" height="250" redraw />
          <h2 style={style}>{`${Math.floor(this.state.result.percent*100)}%`}</h2>
          <ButtonToolbar style={{marginTop: '10px'}}>
            <button onClick={() => {this.transitionTo("defi", this.props.params);}} style={{width: '49%', marginRight: '2%', height:'50px'}} className="green" type="button">Réessayer</button>
            <button onClick={() => {this.transitionTo("challenges");}} style={{width: '49%', height:'50px'}} className="gray" type="button">Nouveau défi</button>
          </ButtonToolbar>
        </Jumbotron>
        </div>
        <div className="col-md-6">
